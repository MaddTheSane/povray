...
-(void) run
{
	...
	{	// keep these grouped, they use NSUserDefaults * defaults
		...
		if ( defaults !=nil)
		{
			...
			{
				if ( [defaults objectForKey:@"displayGamma"] )
					[self addCommand:"Display_Gamma" withString:[NSString stringWithFormat:@"%f",[[defaults objectForKey:@"displayGamma"] floatValue]]];
			}
		}
