...
+ (NSMutableDictionary *) mutableDictionaryWithDefaultSettings
{
	...
	//image & quality ***********************************************************************************
	[NSNumber numberWithInt:cImageSize320x240] ,	@"presetsImageSizes",
	@"320" ,																@"imageSizeX",
	@"240" ,																@"imageSizeY",
	@"1" ,																	@"xSubsetStart",
	@"1" ,																	@"ySubsetStart",
...
/*	SetSubViewsOfNSBoxToState(useIniFileGroup, [useIniInputFile state]);
	if ( [useIniInputFile state]==NSOnState)
		[mUseIniGreenLed setHidden:NO];
	else
		[mUseIniGreenLed setHidden:YES];*/
	//image & quality
	[presetsImageSizes selectItemAtIndex:[[dictToUse objectForKey:@"presetsImageSizes"] intValue]];
...
-(IBAction) preferencesTarget:(id)sender
{
	...
	switch( theTag)
	{
		...
		case cPresetsImageSizes:
		{
			...
			{
				//trim of white spaces, in front is enough here
				x=[[menuItemText stringByTrimmingCharactersInSet: [NSCharacterSet whitespaceCharacterSet]]intValue];
				//find the asterix ('*')
				NSRange asterixStart=[menuItemText rangeOfString:@"*"];
				//start after '*' and trim and take value
				y=[[[menuItemText substringFromIndex: ++asterixStart.location] stringByTrimmingCharactersInSet: [NSCharacterSet whitespaceCharacterSet]]intValue];
				[imageSizeX setIntValue:x];
				[imageSizeY setIntValue:y];
...
-(NSMutableDictionary*) getDictWithCurrentSettings:(BOOL) writeToDefaults
{
	...
	if ( dict)
	{
		...
		//image & quality
		[dict setObject:[NSNumber numberWithInt:[presetsImageSizes indexOfSelectedItem]] forKey:@"presetsImageSizes"];
...
-(void) processSettingsPanel:(int) saveEdit
{
	...
	switch (saveEdit)
	{
		...
		case cModify:
			...
			[self setButtonStateSettingsPanel];
			break;
	}
...
//---------------------------------------------------------------------
-(void) setButtonStateSettingsPanel
{
	int enabledState=NO;
	if(	[settingsPanelTableView numberOfSelectedRows] >0)
	{
		enabledState=YES;
		NSMutableDictionary *dict=[mSettingsArray objectAtIndex:[settingsPanelTableView selectedRow]];
		if ( dict)
		{
			[settingsPanelTextField setStringValue:[dict objectForKey:@"dictionaryName"]];
			[settingsPanelX setStringValue:[dict objectForKey:@"imageSizeX"]];
			[settingsPanelY setStringValue:[dict objectForKey:@"imageSizeY"]];
			if ( [[dict objectForKey:@"samplingOn"] intValue] ==NSOnState)
				[settingsPanelAntiAliasing setStringValue:@"On"];
			else
				[settingsPanelAntiAliasing setStringValue:@"Off"];
			
			[settingsPanelThreshold setStringValue:[dict objectForKey:@"sampleThreshold"]];
			[settingsPanelRecursion setStringValue:[dict objectForKey:@"sampleRecursion"]];
			if ( [[dict objectForKey:@"sampleMethod"] intValue] ==0)
				[settingsPanelMethod setStringValue:@"1"];
			else
				[settingsPanelMethod setStringValue:@"2"];
			[settingsPanelJitter setStringValue:[dict objectForKey:@"sampleJitter"] ];
		}
	}
	else
	{
