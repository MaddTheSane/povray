...
-(void) newSelectionInPreferencesPanelSet:(NSNotification *) notification
{
	...
	if ( mHasSelectionRect)
	{
		...
		mSelectionRectInViewFormat.origin.y/=mBackingScaleFactor;
		mSelectionRectInViewFormat.origin.x/=mBackingScaleFactor;
		mSelectionRectInViewFormat.size.width/=mBackingScaleFactor;
		mSelectionRectInViewFormat.size.height/=mBackingScaleFactor;
...
- (void)mouseDown:(NSEvent *)theEvent
{
	...
	if (mHasSelectionRect==YES)
	{
		...
		mSelectionRectInPixelFormat.origin.y*=mBackingScaleFactor;
		mSelectionRectInPixelFormat.origin.x*=mBackingScaleFactor;
		mSelectionRectInPixelFormat.size.width*=mBackingScaleFactor;
		mSelectionRectInPixelFormat.size.height*=mBackingScaleFactor;
...
-(void) adjustWindow
{
	...
	if ( NSEqualRects(newWindowFrame, oldWindowFrame)==NO)
	{
		...
		if ( [[self window]isVisible]==NO)
			[[self window] orderFront:self];	// display but keep in same level
